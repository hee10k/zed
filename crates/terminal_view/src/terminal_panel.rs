...
impl TerminalPanel {
    ...
    /// Opens (or activates) a center-pane terminal tab running `spawn`,
    /// deduplicated by `spawn.id` across the workspace's center pane group.
    /// Focuses it unless a modal is active.
    pub fn open_or_activate_center_terminal(
        workspace: &mut Workspace,
        spawn: SpawnInTerminal,
        focus: bool,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) -> Task<Result<WeakEntity<TerminalView>>> {
        let existing = workspace.panes().iter().find_map(|pane| {
            pane.read(cx).items().find_map(|item| {
                ...
                (task.spawned_task.id == spawn.id).then_some(terminal_view)
            })
        });
        if let Some(terminal_view) = existing {
            let take_focus = focus && !workspace.has_active_modal(window, cx);
            workspace.activate_item(&terminal_view, true, take_focus, window, cx);
            return Task::ready(Ok(terminal_view.downgrade()));
        }
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            ...
            workspace.update_in(cx, |workspace, window, cx| {
                ...
                let terminal_view_handle = terminal_view.downgrade();
                let take_focus = focus && !workspace.has_active_modal(window, cx);
                workspace.add_item_to_active_pane(
                    Box::new(terminal_view),
                    None,
...
mod tests {
    ...
    #[gpui::test]
    async fn test_open_or_activate_center_terminal_dedupes_by_task_id(
        cx: &mut TestAppContext,
    ) {
        cx.executor().allow_parking();
        init_test(cx);

        let (window_handle, _terminal_panel) = init_workspace_with_panel(cx).await;
        let mut spawn = long_running_task();
        spawn.id = TaskId("center-dedupe".to_owned());
        spawn.cwd = Some(terminal_test_working_directory("center-dedupe"));

        let first = window_handle
            .update(cx, |multi_workspace, window, cx| {
                multi_workspace.workspace().update(cx, |workspace, cx| {
                    TerminalPanel::open_or_activate_center_terminal(
                        workspace,
                        spawn.clone(),
                        true,
                        window,
        ...
        let second = window_handle
            .update(cx, |multi_workspace, window, cx| {
                multi_workspace.workspace().update(cx, |workspace, cx| {
                    TerminalPanel::open_or_activate_center_terminal(
                        workspace, spawn, true, window, cx,
                    )
                })
            })
            .unwrap()
        ...
        let (matching_count, active_item_id) = window_handle
            .read_with(cx, |multi_workspace, cx| {
                let workspace = multi_workspace.workspace().read(cx);
                let matching_count = workspace
                    .panes()
                    .iter()
                    .flat_map(|pane| pane.read(cx).items())
                    .filter_map(|item| item.downcast::<TerminalView>())
                    .filter(|terminal_view| {
                        terminal_view
                            .read(cx)
                            .terminal()
                            .read(cx)
                            .task()
                            .is_some_and(|task| {
                                task.spawned_task.id == TaskId("center-dedupe".into())
                            })
                    })
                    .count();
                let active_item_id = workspace
                    ...
                    .active_item()
                    .and_then(|item| item.downcast::<TerminalView>())
                    .map(|terminal_view| terminal_view.entity_id());
                (matching_count, active_item_id)
            })
            .unwrap();
        ...
        assert_eq!(
            first.entity_id(),
            second.entity_id(),
            "reopening a task activates the existing center terminal"
        );
        assert_eq!(matching_count, 1, "one terminal tab is created per task id");
        assert_eq!(
            active_item_id,
            Some(second.entity_id()),
            "reopening a task activates its center tab"
        );
    }

    #[gpui::test]
    async fn test_open_or_activate_center_terminal_adds_to_center_pane(
        cx: &mut TestAppContext,
    ) {
        cx.executor().allow_parking();
        init_test(cx);
        ...
        let (window_handle, terminal_panel) = init_workspace_with_panel(cx).await;
        let mut spawn = long_running_task();
        spawn.id = TaskId("center-placement".to_owned());
        spawn.cwd = Some(terminal_test_working_directory("center-placement"));
        ...
        let center_items = window_handle
            ...
            .unwrap();
        let dock_items = terminal_panel
            .read_with(cx, |terminal_panel, cx| {
                terminal_panel.active_pane.read(cx).items_len()
            });

        assert_eq!(center_items, 1, "terminal is an item in the center pane group");
        assert_eq!(dock_items, 0, "terminal is not added to the terminal dock");
        assert_eq!(
            ...
            window_handle
                ...
                .unwrap()
                .expect("center terminal should be active"),
            "the created center terminal is active"
        );
    }
