...
Branch, BranchesScanResult, CommitData, CommitDetails, CommitFileStatus, CommitOptions,
        ...
        FileHistoryChangedFileSets, GitCommitTemplate, GitOperationAction, GitOperationKind,
        GitRepository, GitRepositoryCheckpoint, InitialGraphCommitData, LogOrder, LogSource,
        MergeMode, PushOptions, Remote, RemoteCommandOutput, RepoPath, ResetMode, SearchCommitArgs,
    ...
    pub fn load_commit_range_diff(
        ...
        self.send_job(
            ...
            move |git_repo, cx| async move {
                match git_repo {
                    RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
                        let diff = backend.load_commit_range(base, target, cx).await?;
                        Ok(decode_commit_diff(diff))
                    }
                    RepositoryState::Remote(RemoteRepositoryState {
                        client, project_id, ..
                    }) => {
...
fn git_operation_kind_from_proto(proto_val: i32) -> GitOperationKind {
    match proto::GitOperationKind::try_from(proto_val) {
        Ok(proto::GitOperationKind::MergeOperation) | Err(_) => GitOperationKind::Merge,
        Ok(proto::GitOperationKind::RebaseOperation) => GitOperationKind::Rebase,
        Ok(proto::GitOperationKind::CherryPickOperation) => GitOperationKind::CherryPick,
        Ok(proto::GitOperationKind::RevertOperation) => GitOperationKind::Revert,
    }
}
...
fn git_operation_action_from_proto(proto_val: i32) -> GitOperationAction {
    match proto::GitOperationAction::try_from(proto_val) {
        Ok(proto::GitOperationAction::ContinueAction) | Err(_) => GitOperationAction::Continue,
        Ok(proto::GitOperationAction::SkipAction) => GitOperationAction::Skip,
        Ok(proto::GitOperationAction::AbortAction) => GitOperationAction::Abort,
    }
}
